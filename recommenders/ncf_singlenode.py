...
class NCF:
    ...
    def _create_model(
        ...
        with tf.compat.v1.variable_scope("loss", reuse=tf.compat.v1.AUTO_REUSE):
            ...
            # set loss function (zs changed to mse)
            self.loss = tf.compat.v1.losses.mean_squared_error(self.labels, self.output)
        ...
        with tf.compat.v1.variable_scope("optimizer", reuse=tf.compat.v1.AUTO_REUSE):
            ...
            except tf.error as e:
                print("Error message:", e)
                with tf.Session() as sess:
                    mul_5_values = sess.run('embedding_mlp_Q/mul_5:0')
                    print("embedding_mlp_Q/mul_5 values:", mul_5_values)
